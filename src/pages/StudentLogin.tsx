...
const StudentLogin = () => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    const result = loginUser(data.email, data.password, "client");
    if (!result.success || !result.profile) {
      setError("root", { message: result.error ?? "Error al iniciar sesión." });
      return;
    }
    login("mock-token-student", result.profile);
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
              ...
              <div className="space-y-2 pt-2 text-center text-sm text-muted-foreground">
                <p>
                  ¿No tienes cuenta?{" "}
                  <Link to="/accounts/register/client" className="text-primary hover:underline">
